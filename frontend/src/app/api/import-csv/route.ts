...
export async function GET(request: Request) {
  try {
    ...
    // Виправлено шлях для нової структури (frontend/data -> ../data)
    ...
    if (!fs.existsSync(filePath)) {
      ...
      return NextResponse.json(
        { error: 'CSV файл не знайдено. Перевірте що файл data/match_results.csv існує' },
        { status: 404 }
      );
    }
      ...
      for (let run = 1; run <= warmupRuns; run++) {
        ...
        sortedRows.forEach((row) => {
          ...
          const { player1Change, player2Change } = calculateRatingChange(
            ...
            gamesPlayed1,
            gamesPlayed2,
            1.0, // matchWeight (deprecated, для backward compatibility)
            row.stage // 🔥 Передаємо стадію для асиметричних множників
          );
